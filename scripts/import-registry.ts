/**
 * YETO Source Registry Importer — CANONICAL SCRIPT
 *
 * This is the ONLY script that should be used to import source data.
 * It reads the canonical xlsx from data/registry/ and upserts into source_registry.
 *
 * Usage:
 ...
 * - Old scripts archived to archive/deprecated-scripts/
 */
...
// Find the xlsx file: use --file arg, or auto-detect latest in data/registry/
function findExcelPath(): string {
  // Check for --file argument
  const fileArgIdx = process.argv.indexOf('--file');
  if (fileArgIdx !== -1 && process.argv[fileArgIdx + 1]) {
    const customPath = path.resolve(process.argv[fileArgIdx + 1]);
    if (fs.existsSync(customPath)) return customPath;
    throw new Error(`File not found: ${customPath}`);
  }

  // Auto-detect: find the latest xlsx in data/registry/
  const registryDir = path.join(__dirname, '../data/registry');
  if (fs.existsSync(registryDir)) {
    const xlsxFiles = fs.readdirSync(registryDir)
      .filter(f => f.endsWith('.xlsx') && f.includes('YETO_Sources'))
      .sort()
      .reverse();
    if (xlsxFiles.length > 0) {
      return path.join(registryDir, xlsxFiles[0]);
    }
  }
  ...
  if (rootFiles.length > 0) {
    return path.join(rootDir, rootFiles[0]);
  }

  throw new Error(
    'No YETO Sources xlsx found. Place it in data/registry/ or pass --file path/to/file.xlsx'
  );
}
