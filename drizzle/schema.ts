...
// ============================================================================
// SUBSCRIPTION MANAGEMENT - COMPREHENSIVE SYSTEM
// ============================================================================
 ...
 */
export const notificationPreferencesEnhanced = mysqlTable("notification_preferences_enhanced", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").references(() => users.id),
  email: varchar("email", { length: 320 }).notNull(),
  
  // Exchange Rate Alerts
  exchangeRateAlerts: boolean("exchangeRateAlerts").default(true).notNull(),
  exchangeRateThreshold: decimal("exchangeRateThreshold", { precision: 5, scale: 2 }).default("2.00"), // % change to trigger
  exchangeRateRegions: json("exchangeRateRegions").$type<('aden' | 'sanaa' | 'both')[]>().default(['both']),
  
  // Sector Alerts
  bankingAlerts: boolean("bankingAlerts").default(true).notNull(),
  tradeAlerts: boolean("tradeAlerts").default(true).notNull(),
  energyAlerts: boolean("energyAlerts").default(true).notNull(),
  foodSecurityAlerts: boolean("foodSecurityAlerts").default(true).notNull(),
  humanitarianAlerts: boolean("humanitarianAlerts").default(true).notNull(),
  fiscalAlerts: boolean("fiscalAlerts").default(true).notNull(),
  macroeconomicAlerts: boolean("macroeconomicAlerts").default(true).notNull(),
  
  // Publication Notifications
  newPublicationsAlert: boolean("newPublicationsAlert").default(true).notNull(),
  publicationCategories: json("publicationCategories").$type<string[]>(),
  publicationOrganizations: json("publicationOrganizations").$type<number[]>(),
  
  // Report Delivery
  weeklyDigest: boolean("weeklyDigest").default(true).notNull(),
  weeklyDigestDay: mysqlEnum("weeklyDigestDay", ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"]).default("monday"),
  monthlyReport: boolean("monthlyReport").default(true).notNull(),
  monthlyReportDay: int("monthlyReportDay").default(1), // Day of month
  quarterlyReport: boolean("quarterlyReport").default(true).notNull(),
  annualReport: boolean("annualReport").default(true).notNull(),
  
  // Format Preferences
  preferredLanguage: mysqlEnum("preferredLanguage", ["en", "ar", "both"]).default("both").notNull(),
  reportFormat: mysqlEnum("reportFormat", ["pdf", "html", "both"]).default("pdf").notNull(),
  includeCharts: boolean("includeCharts").default(true).notNull(),
  includeDataTables: boolean("includeDataTables").default(true).notNull(),
  
  // Delivery Preferences
  deliveryFrequency: mysqlEnum("deliveryFrequency", ["immediate", "daily_digest", "weekly_digest"]).default("daily_digest").notNull(),
  quietHoursStart: int("quietHoursStart"), // Hour (0-23)
  quietHoursEnd: int("quietHoursEnd"), // Hour (0-23)
  ...
  emailIdx: index("notification_email_idx").on(table.email),
  isActiveIdx: index("notification_active_idx").on(table.isActive),
  verificationTokenIdx: index("notification_verification_idx").on(table.verificationToken),
}));

export type NotificationPreferenceEnhanced = typeof notificationPreferencesEnhanced.$inferSelect;
export type InsertNotificationPreferenceEnhanced = typeof notificationPreferencesEnhanced.$inferInsert;

/**
 * Scheduled Report Deliveries
 * Track scheduled reports and their delivery status
 */
export const scheduledReportDeliveries = mysqlTable("scheduled_report_deliveries", {
  id: int("id").autoincrement().primaryKey(),
  ...
  reportTitleAr: varchar("reportTitleAr", { length: 255 }),
  
  // Generated Report Files
  pdfFileKey: varchar("pdfFileKey", { length: 255 }),
  pdfFileUrl: text("pdfFileUrl"),
  htmlFileKey: varchar("htmlFileKey", { length: 255 }),
  htmlFileUrl: text("htmlFileUrl"),
  ...
  deliveryCompletedAt: timestamp("deliveryCompletedAt"),
  
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, (table) => ({
  reportTypeIdx: index("scheduled_report_type_idx").on(table.reportType),
  scheduledForIdx: index("scheduled_for_idx").on(table.scheduledFor),
  generationStatusIdx: index("generation_status_idx").on(table.generationStatus),
}));

export type ScheduledReportDelivery = typeof scheduledReportDeliveries.$inferSelect;
export type InsertScheduledReportDelivery = typeof scheduledReportDeliveries.$inferInsert;
...
export const emailDeliveryLog = mysqlTable("email_delivery_log", {
  ...
  recipientEmail: varchar("recipientEmail", { length: 320 }).notNull(),
  recipientName: varchar("recipientName", { length: 255 }),
  notificationPreferenceId: int("notificationPreferenceId").references(() => notificationPreferencesEnhanced.id),
  ...
  // Related Report
  scheduledReportId: int("scheduledReportId").references(() => scheduledReportDeliveries.id),
  
  // Delivery Status
  status: mysqlEnum("status", ["pending", "sent", "delivered", "bounced", "failed", "opened", "clicked"]).default("pending").notNull(),
  sentAt: timestamp("sentAt"),
  deliveredAt: timestamp("deliveredAt"),
  openedAt: timestamp("openedAt"),
  clickedAt: timestamp("clickedAt"),
  ...
  providerResponse: json("providerResponse").$type<Record<string, unknown>>(),
  
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, (table) => ({
  recipientIdx: index("delivery_recipient_idx").on(table.recipientEmail),
  emailTypeIdx: index("delivery_type_idx").on(table.emailType),
  statusIdx: index("delivery_status_idx").on(table.status),
  sentAtIdx: index("delivery_sent_idx").on(table.sentAt),
  scheduledReportIdx: index("delivery_report_idx").on(table.scheduledReportId),
}));
...
/**
 * Alert Queue
 * Queue for pending alerts to be sent
 */
export const alertQueue = mysqlTable("alert_queue", {
  ...
  // Related Data
  indicatorCode: varchar("indicatorCode", { length: 100 }),
  region: mysqlEnum("region", ["aden", "sanaa", "both"]),
  ...
  // Deduplication
  deduplicationKey: varchar("deduplicationKey", { length: 255 }),
  
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  expiresAt: timestamp("expiresAt"), // Don't send if expired
}, (table) => ({
  alertTypeIdx: index("queue_alert_type_idx").on(table.alertType),
  statusIdx: index("queue_status_idx").on(table.status),
  priorityIdx: index("queue_priority_idx").on(table.priority),
  deduplicationIdx: index("queue_dedup_idx").on(table.deduplicationKey),
}));
 ...
 * Organizations that receive reports
 */
export const institutionalSubscribers = mysqlTable("institutional_subscribers", {
  id: int("id").autoincrement().primaryKey(),
  
  // Organization Info
  organizationName: varchar("organizationName", { length: 255 }).notNull(),
  organizationNameAr: varchar("organizationNameAr", { length: 255 }),
  organizationType: mysqlEnum("organizationType", [
    "donor",
    "un_agency",
    "ngo",
  ...
  secondaryContactEmail: varchar("secondaryContactEmail", { length: 320 }),
  
  // Subscription Details
  subscriptionTier: mysqlEnum("subscriptionTier", ["basic", "professional", "enterprise"]).default("basic").notNull(),
  customReportAccess: boolean("customReportAccess").default(false).notNull(),
  apiAccess: boolean("apiAccess").default(false).notNull(),
  
  // Report Preferences
  receiveWeeklyDigest: boolean("receiveWeeklyDigest").default(true).notNull(),
  receiveMonthlyReport: boolean("receiveMonthlyReport").default(true).notNull(),
  receiveQuarterlyReport: boolean("receiveQuarterlyReport").default(true).notNull(),
  receiveAnnualReport: boolean("receiveAnnualReport").default(true).notNull(),
  preferredLanguage: mysqlEnum("preferredLanguage", ["en", "ar", "both"]).default("both").notNull(),
  
  // Sector Interests
  interestedSectors: json("interestedSectors").$type<string[]>(),
  interestedIndicators: json("interestedIndicators").$type<string[]>(),
  ...
  organizationNameIdx: index("inst_org_name_idx").on(table.organizationName),
  organizationTypeIdx: index("inst_org_type_idx").on(table.organizationType),
  primaryEmailIdx: index("inst_primary_email_idx").on(table.primaryContactEmail),
  isActiveIdx: index("inst_active_idx").on(table.isActive),
}));

export type InstitutionalSubscriber = typeof institutionalSubscribers.$inferSelect;
export type InsertInstitutionalSubscriber = typeof institutionalSubscribers.$inferInsert;
