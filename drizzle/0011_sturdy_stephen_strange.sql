...
ALTER TABLE `insight_miner_proposals` ADD CONSTRAINT `insight_miner_proposals_incorporatedInReportId_report_instances_id_fk` FOREIGN KEY (`incorporatedInReportId`) REFERENCES `report_instances`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
